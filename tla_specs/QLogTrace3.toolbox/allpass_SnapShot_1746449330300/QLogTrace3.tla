...
Safety_PktNosMonotonic ==
    LET 
        cseq == [n \in 1.. Cardinality (SentEvents (ClientTrace, ci-1)) |-> 
        CHOOSE i \in SentEvents (ClientTrace, ci-1): Cardinality({j \in SentEvents (ClientTrace, ci-1): j < i}) = n - 1] 
            sseq == [n \in 1..Cardinality (SentEvents (ServerTrace, si-1)) |-> 
        CHOOSE i \in SentEvents (ServerTrace, si-1): Cardinality({j \in SentEvents (ServerTrace, si-1): j < i}) = n - 1] 
    IN 
        (Cardinality(cseq) < 2 \ (\A n \in 2..Cardinality (cseq): ClientTrace[cseq[n]].data.header.packet_number > ClientTrace [cseq [n-1]].data.header.packet_number)) /\
        (Cardinality(sseq) < 2 \ (\A n \in 2.. Cardinality(sseq): ServerTrace[sseq[n]].data.header.packet_number > ServerTrace [sseq [n-1]].data.header.packet_number))
